...
if [ -e "/root/start.txt" ];then
                if [ -e "/root/mods/fabric-api-0.91.6+1.20.2.jar" ];then
                         echo "此文件已经准备好"
                else
                         echo "此文件未准备，正在准备"
                         mv fabric-api-0.91.6+1.20.2.jar /root/mods/fabric-api-0.91.6+1.20.2.jar
                         echo "文件已经准备好"
                fi
                if [ -e "/root/mods/geyser-fabric-2.2.0-SNAPSHOT+build.393.jar" ];then
                         echo "此文件已经准备好"
                else
                         echo "此文件未准备，正在准备"
                         mv geyser-fabric-2.2.0-SNAPSHOT+build.393.jar /root/mods/geyser-fabric-2.2.0-SNAPSHOT+build.393.jar
                         echo "文件已经准备好"
                fi
else
                touch start.txt
